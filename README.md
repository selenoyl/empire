...
### Windows double-click friendly (recommended)
```bat
...
- `build-release.ps1` now pauses by default so errors are readable; use `-NoPause` for CI/non-interactive runs.
- `scripts/build-release.bat` is provided for easiest Windows launch and always preserves console output (`NO_PAUSE=1` to disable).

Artifacts are written to:
- `artifacts/build-release-last.log` (latest build transcript/errors)
