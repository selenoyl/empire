...
### Double-click friendly (Windows)
```cmd
scripts\build-release.cmd
```
This wrapper uses PowerShell `-ExecutionPolicy Bypass` and keeps the console open so errors are visible.

- PowerShell script pauses by default so you can read failures. Use `-NoPause` in CI or non-interactive shells.

Build logs are written to:
- `artifacts/build-release.log`
