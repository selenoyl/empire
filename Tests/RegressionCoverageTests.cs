...
public class RegressionCoverageTests
{
    ...
    public void ReplayDeserializer_ParsesExpandedCommandSet()
    {
        ...
        {
            JsonSerializer.Serialize(new SetCityProductionCommand(1, 2, "Warrior")),
            JsonSerializer.Serialize(new SetFortifyCommand(1, 3, true)),
            JsonSerializer.Serialize(new BuildImprovementCommand(1, 4, ImprovementType.Farm)),
            JsonSerializer.Serialize(new SetWarPeaceCommand(1, 2, DiplomacyState.Peace)),
            JsonSerializer.Serialize(new ProposeTradeCommand(1, 2, 3, 10)),
            JsonSerializer.Serialize(new AcceptOpenBordersCommand(1, 2, true)),
        };
